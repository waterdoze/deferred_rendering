...
void Shape::loadMesh(const string &meshName, bool isPlane)
{
	// Load geometry
		...
		for(size_t s = 0; s < shapes.size(); s++) {
			...
			for(size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
				...
				for(size_t v = 0; v < fv; v++) {
					...
					posBuf.push_back(isPlane ? 0 : attrib.vertices[3*idx.vertex_index+1]);
                    if (attrib.vertices[3*idx.vertex_index+1] < lowest) {
                        lowest = attrib.vertices[3*idx.vertex_index+1];
                    }
					posBuf.push_back(attrib.vertices[3*idx.vertex_index+2]);
					if(!attrib.normals.empty()) {
						norBuf.push_back(isPlane ? 0 : attrib.normals[3*idx.normal_index+0]);
						norBuf.push_back(isPlane ? 1 : attrib.normals[3*idx.normal_index+1]);
						norBuf.push_back(isPlane ? 0 : attrib.normals[3*idx.normal_index+2]);
					}
...
void Shape::insertBuffers(std::vector<float> posBuf, std::vector<float> norBuf, std::vector<unsigned int> indBuf) {
    this->posBuf = posBuf;
    this->norBuf = norBuf;
    this->indBuf = indBuf;
}
...
void Shape::init()
{
	...
	if(!indBuf.empty()) {
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, indBuf.size()*sizeof(int), &indBuf[0], GL_STATIC_DRAW);
	}
...
void Shape::draw(const shared_ptr<Program> prog, bool revolves) const
{
